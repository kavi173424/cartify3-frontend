...
export class AdminAuthComponent {
  ...
  register(){ this.auth.register({...this.reg, role:'Admin'}).subscribe(r=>{ this.auth.saveAuth(r.token, r.role); this.router.navigateByUrl('/admin/dashboard'); }); }
  login(){ this.auth.login(this.log).subscribe(r=>{ this.auth.saveAuth(r.token, r.role); this.router.navigateByUrl('/admin/dashboard'); }); }
}

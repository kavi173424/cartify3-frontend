...
export class UserAuthComponent {
  ...
  register(){ this.auth.register({...this.reg, role:'User'}).subscribe(r=>{ this.auth.saveAuth(r.token, r.role); this.router.navigateByUrl('/shop'); }); }
  login(){ this.auth.login(this.log).subscribe(r=>{ this.auth.saveAuth(r.token, r.role); this.router.navigateByUrl('/shop'); }); }
}
